    ...
    async (acceptedFiles: File[]) => {
      ...
      if (quota) {
        if (quota.file_count >= quota.max_files) {
          setError(
            `You have reached your file limit (${quota.max_files} files). Delete some files or upgrade your plan.`,
          )
          onUploadError?.(`File limit reached (${quota.max_files} files)`)
          return
        }

        if (quota.used_bytes + file.size > quota.max_bytes) {
          const remainingMB = (
            (quota.max_bytes - quota.used_bytes) /
            1024 /
            1024
          ).toFixed(2)
          setError(
      ...
      if (file.size > maxSize) {
        setError(
          `File is too large. Maximum size is 10 MB, but this file is ${(file.size / 1024 / 1024).toFixed(2)} MB.`,
        )
