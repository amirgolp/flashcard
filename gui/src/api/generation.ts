...
export async function generateFromRange(
  req: GenerateFromRangeRequest,
): Promise<GenerationResponse> {
  const { data } = await apiClient.post<GenerationResponse>(
    '/generate/from-range',
    req,
...
export async function listDrafts(params: {
  ...
  const { data } = await apiClient.get<DraftCardResponse[]>(
    '/generate/drafts',
    { params },
  )
  return data
...
export async function updateDraft(
  id: string,
  draft: DraftCardUpdate,
): Promise<DraftCardResponse> {
  const { data } = await apiClient.put<DraftCardResponse>(
    `/generate/drafts/${id}`,
    draft,
  )
  return data
