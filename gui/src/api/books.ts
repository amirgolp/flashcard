...
export async function listBooks(skip = 0, limit = 20): Promise<BookResponse[]> {
  const { data } = await apiClient.get<BookResponse[]>('/books/', {
    params: { skip, limit },
  })
  return data
}
...
export async function getBookProgress(
  ...
  const { data } = await apiClient.get<BookProgressResponse>(
...
export async function updateBookProgress(
  ...
  const { data } = await apiClient.put<BookProgressResponse>(
    `/books/${bookId}/progress`,
    progress,
  )
  return data
...
export async function updateBookChapters(
  ...
  const { data } = await apiClient.put<BookResponse>(
