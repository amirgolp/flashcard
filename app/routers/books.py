...
def create_book(
    ...
    """
    Register a book whose PDF lives on the client device. The server
    only stores metadata (title, page count, languages, optional
    chapters) and uses storage_type='device' to mark it as not having
    a server-side file. Cards are subsequently generated by sending
    base64 page images via /generate/from-images.
    """
    return crud.create_book_metadata(book_create, db=db, owner=current_user)
